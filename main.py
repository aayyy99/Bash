    ...
    try:
        ...
        for source_url in source_urls:
            ...
            try:
                ...
                print(f"成功获取链接内容：{source_url}")

                if source_url.endswith('.m3u'):
                    print(f"开始解析 M3U 文件：{source_url}")
                    ...
                    for line in content.splitlines():
                        if line.startswith('#EXTM3U'):
                            ...
                            print(f"提取到频道：{current_channel_name}, 地址：{line.strip()}")
                            current_channel_name = "" # 重置频道名称
                    print(f"M3U 文件解析完成：{source_url}")
                elif source_url.endswith('.txt'):
                    print(f"开始解析 TXT 文件：{source_url}")
                    for line in content.splitlines():
                        if line.startswith('http'):
                            iptv_data.append(("", line.strip()))
                            print(f"提取到地址：{line.strip()}")
                    print(f"TXT 文件解析完成：{source_url}")
